function LCtable = dataprocess1_TIA(recordingMetaData,tracksData, ...
    tracksMetaData)
...
lowerdata = dataprocess2_lowerdata_coordinateSystem(lowerdata, ...
    recordingMetaData);
%% 开始提取换道数据
upperLCdata = dataprocess3_LC_TIA(upperdata,recordingMetaData, ...
    tracksMetaData);
lowerLCdata = dataprocess3_LC_TIA(lowerdata,recordingMetaData, ...
    tracksMetaData);
LCdata = [upperLCdata;lowerLCdata];
% %% 开始提取直行数据
% upperLKdata = dataprocess3_LK(upperdata,recordingMetaData,
% tracksMetaData);
% lowerLKdata = dataprocess3_LK(lowerdata,recordingMetaData,
% tracksMetaData);
% LKdata = [upperLKdata;lowerLKdata];
%% 合并LC,LK为表格
% LCKdata = [LCdata;LKdata];
LCtable = cell2table(LCdata,"VariableNames",{'Samples','Label'});