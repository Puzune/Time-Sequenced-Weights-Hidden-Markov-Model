function LCdata = dataprocess3_LC_TIA(data,recordingMetaData, ...
    tracksMetaData)
...
for i = 1:numlc
    ...
    if isempty(LCinipoint)
        continue
    elseif LCinipoint<100
        continue
    end
    % 检查换道过程数据是否够长
    if (height(cardata)-LCinipoint+1)<50
        continue
    end

    % 判断换道方向
    if cardata.HeadingAngle(end)<0
        flag = 1;
    elseif cardata.HeadingAngle(end)>0
        flag = 2;
    else
        continue
    end
